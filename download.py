...
def download(url, make_m3u=False, name="", threads=THREADS, retries=MAX_RETRIES):
    ...
    try:
        ...
        if make_m3u:
            ...
            command = f"spotdl --audio youtube-music soundcloud --no-cache --max-retries {retries} --threads {threads} --bitrate 128k --format mp3 --output OUTPUT --overwrite metadata --restrict ascii --print-errors --scan-for-songs --preload --create-skip-file --respect-skip-file --log-level DEBUG --simple-tui --profile download {url}"
