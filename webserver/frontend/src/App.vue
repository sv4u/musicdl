      ...
      <div class="bg-slate-800 rounded-lg border border-slate-700 shadow-lg">
        <div class="flex border-b border-slate-700">
          <button
            v-for="tab in tabs"
            :key="tab"
            :class="['px-6 py-3 font-medium transition-colors', activeTab === tab ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-white']"
            @click="activeTab = tab"
          >
            {{ tab }}
          </button>
...
const activeTab = ref<'Download' | 'Configuration' | 'Logs'>('Download');
const tabs = ['Download', 'Configuration', 'Logs'];
